...
::: tip
原定商品列表是打算包含在用户主页的，但是商品列表，问答列表这些如果都和动态横纵组合滑动，会显得比较臃肿，此外用户的礼物墙也是本应用中一个比较矛盾的地方，所以把礼物榜融入到魅力值中，以突出用户特性为主，弱化其他环节。
:::

用户主页的最终UI设计如下：

![iPhone X-XS-11 Pro – 22.png](http://blog-img.molifouru.cn/iPhone%20X-XS-11%20Pro%20–%2022.png)

___

### [IM聊天(茉栗IM)](../)

私信的聊天页面，单独设计了一套统一标准的聊天页面，未来的应用中也会用这一套IM界面，这个APP的聊天页面的功能将是最全面的。完整的IM功能将会独立成为[茉栗IM SDK](../)
...
![1615263516079.png](http://blog-img.molifouru.cn/1615263516079.png)
...
这个问题我思考了很久，我只能说，有的时候，语音气泡真的需要一个暂停按钮。此外，这样设计出来也确实蛮好看的。
:::
...
工具栏将作为IM开发的核心功能，增加IM的互动性。以工具组件的形式扩展功能，包含小游戏等等。用户可以对工具栏中的工具进行排序和调整，或添加自己想要的小工具。小工具可以对接APP的不同业务类型。对于小工具的业务逻辑和功能，将在后续博文中延伸。
